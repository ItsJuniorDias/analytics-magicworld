...
## ⚠️ First: the leaked ADMIN_TOKEN

`.env` was committed to this repository, and the repository is **public**
(`github.com/ItsJuniorDias/analytics-magicworld`). The file is readable by
anyone, without logging in. The `ADMIN_TOKEN` inside it is the only thing
protecting `/admin/*` — every event, every user id, the whole funnel.
...
That is why the dashboard has a *"Still have access"* card: those are the
...
`subscriptions` table. `sub_cancelled` is always the total; `sub_trial_cancelled`
is the subset, emitted alongside it and never in its place, so "how many
cancelled" stays a single sum.
...
4. First deploy pins Node 22.11 (`NODE_VERSION` env + `.node-version` — the
   latter was referenced by this README but missing from the repo until now;
   `NODE_VERSION` in `render.yaml` was doing the work alone).
