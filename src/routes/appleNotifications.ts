  ...
  app.post<{ Body: { signedPayload?: unknown } }>(
    ...
    async (req, reply) => {
      ...
      if (typeof signedPayload !== "string") {
        return reply
          .code(400)
          .send({ ok: false, error: "missing_signed_payload" });
      }
      ...
      try {
        if (notification.data?.signedTransactionInfo) {
          tx = await verifier.verifyTransaction(
            notification.data.signedTransactionInfo,
          );
        }
        if (notification.data?.signedRenewalInfo) {
          renewal = await verifier.verifyRenewalInfo(
      ...
      // than anything the device reports, which is only where they opened the
      // app. Converted to alpha-2 so it lines up with the app's own events and
      // renders as a flag on the dashboard.
      const country = storefrontToAlpha2(tx.storefront);
      const price =
        fromMilliunits(tx.price) ?? fromMilliunits(renewal.renewalPrice);
      ...
      req.log.info(
        { type, subtype, events: names, productId },
        "apple_notification_handled",
      );
