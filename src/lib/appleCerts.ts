 ...
 *
 * So the .cer files are NOT bundled: they are public Apple binaries, fetched
 * by `npm run certs:apple`. With the folder empty the server still boots, logs
 * loudly, and the webhook answers 503 — never 200 for unverified data.
 *
 * Alternative for people who would rather not commit files: APPLE_ROOT_CERTS_B64
 * with base64 certificates, comma separated (fits in a Render env var).
 */
